...
import { deleteAdminImagePrankCategory, updateAdminImagePrankCategory } from '@/server/admin/image-pranks';
...
export const PATCH = withApiError(async function PATCH(
  ...
  const updated = await updateAdminImagePrankCategory(id, {
  ...
  return ok(updated);
}, 'Failed to update image prank category');
...
export const DELETE = withApiError(async function DELETE(
  ...
  await deleteAdminImagePrankCategory(id, deleteFiles);
  return ok({ ok: true });
}, 'Failed to delete image prank category');
