...
import {
  createAdminImagePrankSubcategory,
  listAdminImagePrankSubcategories,
...
export const GET = withApiError(async function GET(req: NextRequest) {
  ...
  return ok({ items: await listAdminImagePrankSubcategories({ categoryId: categoryId || null }) });
}, 'Failed to list image prank subcategories');
...
export const POST = withApiError(async function POST(req: NextRequest) {
  ...
  const created = await createAdminImagePrankSubcategory({
    categoryId,
    slug,
    title,
    subtitle,
...
}, 'Failed to create image prank subcategory');
