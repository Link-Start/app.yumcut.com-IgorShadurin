...
import { checkAdminCharacterPriorities } from '@/server/admin/characters';
...
export const POST = withApiError(async function POST(req: NextRequest) {
  ...
  const result = await checkAdminCharacterPriorities({
    categoryId,
    slugs,
  });
  return ok(result);
}, 'Failed to check admin character priorities');
