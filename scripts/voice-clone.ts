...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    const value = inlineValue ?? takeNextValue(argv, index, raw);
...
async function main() {
  ...
  const response = await callRunpod(endpointId, apiKey, payload, options);
  ...
  await fs.writeFile(timingPath, buildTimingText({ endpointId, requestId: options.requestId, elapsedMs, outputPath: outPath, artifact, timing: output.timing }), 'utf8');
